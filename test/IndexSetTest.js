...
class IndexSetTest extends SetTest {
  ...
  test_addIndex(test) {
    it('{}[] + index["name"} +  -> true', function() {
      let index_set = test.newInstance();
      assert(index_set.addIndex(['name'] == true));
    });
    it('{}[["name]] + index["name"} -> false', function() {
      let index_set = test.newInstance();
      index_set.addIndex(['name']);
      assert(index_set.addIndex(['name'] == false));
    });
    it('{}[["name]] + index["age"} -> true', function() {
      let index_set = test.newInstance();
      index_set.addIndex(['name']);
      assert(index_set.addIndex(['age'] == true));
    });
    it('{p1,p2}[] + index["name"} -> true', function() {
      let index_set = test.newInstance();
